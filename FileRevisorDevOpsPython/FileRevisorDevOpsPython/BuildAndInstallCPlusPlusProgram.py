...
def linux_cmake_build_test_install(cmakeBuildType: str, testsProjectName: str, cmakeDefinitions: str, doInstallProgram: bool) -> None:
   ...
   zenUnitTestsProgramCommand = f'{testsProjectName}/{testsProjectName} --test-runs=2 --random --max-test-milliseconds=500 --exit-1-if-tests-skipped'
...
def windows_cmake_build_test_install(solutionName: str, cmakeBuildType: str, testsProjectName: str, cmakeDefinitions: str, doInstallProgram: bool) -> None:
   ...
   msbuildCommand = f'MSBuild.exe {solutionName}.slnx /p:Configuration={cmakeBuildType} /p:Platform=x64 /m'
   Process.fail_fast_run(msbuildCommand)
   zenUnitTestsProgramCommand = f'{testsProjectName}/{cmakeBuildType}/{testsProjectName}.exe --test-runs=2 --random --max-test-milliseconds=500 --exit-1-if-tests-skipped'
   Process.fail_fast_run(zenUnitTestsProgramCommand)
...
# --solution-name=FileRevisor --cmake-build-type=Debug --tests-project=libFileRevisorTests --no-install
