...
Usage: BuildAndInstallCPlusPlusProgram.py --solution-name=<String> --cmake-build-type=<String> --tests-project-name=<String> [--cmake-definitions=<String>] (--install|--no-install)""",
   ...
   def test_linux_cmake_build_test_install__RunCMakeThenRunsNinja(self, _1, _2, _3, _4):
      ...
      expectedZenUnitTestsProgramCommand = f'{self.testsProjectName}/{self.testsProjectName} --test-runs=2 --random --max-test-milliseconds=500 --exit-1-if-tests-skipped'
   ...
   def test_windows_cmake_build_test_install__RunsCMakeThenRunsMSBuild(self, _1, _2, _3):
      ...
      expectedZenUnitTestsProgramCommand = f'{self.testsProjectName}/{self.cmakeBuildType}/{self.testsProjectName}.exe --test-runs=2 --random --max-test-milliseconds=500 --exit-1-if-tests-skipped'
      self.assertEqual(2, len(Process.fail_fast_run.call_args_list))
      Process.fail_fast_run.assert_has_calls([
