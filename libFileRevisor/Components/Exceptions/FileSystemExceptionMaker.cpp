...
FileSystemException FileSystemExceptionMaker::
MakeFileSystemExceptionForFailedToRenameDirectory(
   const fs::path& directoryPath,
   const fs::path& renamedDirectoryPath,
   const error_code& renameErrorCode) const
{
...
FileSystemException FileSystemExceptionMaker::
MakeFileSystemExceptionForFailedToDeleteFile(const char* filePath) const
{
   const pair<int, string> errnoWithDescription = _errorCodeTranslator->GetErrnoWithDescription();
...
FileSystemException FileSystemExceptionMaker::
MakeFileSystemExceptionForFailedToSetFileAttribute(const char* filePath, DWORD fileAttributes) const
{
   const pair<DWORD, string> windowsLastErrorWithDescription =
      _errorCodeTranslator->GetWindowsLastErrorWithDescription();
   const string exceptionMessage = String::Concat(
