...
#pragma warning(pop)
	...
	static std::vector<std::string> ParseSection(const std::string& name, const std::string& source)
	{
      ...
      static const std::regex sectionRegexPattern
      {
		...
		std::vector<std::string> matchingSectionStrings;
      const auto regexIterator = std::sregex_iterator(begin(source), end(source), sectionRegexPattern);
		std::for_each(regexIterator, std::sregex_iterator(), [&](const std::smatch& match)
		{
         std::string matchString = match[1].str();
         std::string trimmedMatchString = trim(std::move(matchString));
         matchingSectionStrings.emplace_back(std::move(trimmedMatchString));
		});
		return matchingSectionStrings;
	}
...
#pragma warning(pop)
		...
		bool options_first) noexcept
	{
		try
		{
         const std::map<std::string, Value> docoptArguments = docopt_parse(doc, argv, help, !version.empty(), options_first);
